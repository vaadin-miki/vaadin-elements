...
class Vaadin::GridTest < Minitest::Test
  ...
  def test_choices_immediate
    ...
    assert_equal "<vaadin-grid id=\"grid\"></vaadin-grid><script async=\"false\" defer=\"true\">document.addEventListener(\"WebComponentsReady\", function(e) {var cb = document.querySelector(\"#grid\");cb.items = [{\"code\":\"PL\",\"english\":\"Poland\",\"name\":\"Polska\"},{\"code\":\"FI\",\"english\":\"Finland\",\"name\":\"Suomi\"},{\"code\":\"DE\",\"english\":\"Germany\",\"name\":\"Deutschland\"},{\"code\":\"SE\",\"english\":\"Sweden\",\"name\":\"Sverige\"}];cb.addEventListener('selected-items-changed', function(e) {selection = document.querySelector(\"#grid\").selection.selected(function(index){document.querySelector(\"#grid\").getItem(index, function(err, item){return index;});});ajax.post('/grid', {id: 'grid', value: selection}, null);});});</script>", html
  end

  def test_choices_immediate_value_path
    html = vaadin_grid @countries, id: 'grid', immediate: true, item_value_path: 'code'
    assert_equal "<vaadin-grid id=\"grid\"></vaadin-grid><script async=\"false\" defer=\"true\">document.addEventListener(\"WebComponentsReady\", function(e) {var cb = document.querySelector(\"#grid\");cb.items = [{\"code\":\"PL\",\"english\":\"Poland\",\"name\":\"Polska\"},{\"code\":\"FI\",\"english\":\"Finland\",\"name\":\"Suomi\"},{\"code\":\"DE\",\"english\":\"Germany\",\"name\":\"Deutschland\"},{\"code\":\"SE\",\"english\":\"Sweden\",\"name\":\"Sverige\"}];cb.addEventListener('selected-items-changed', function(e) {selection = document.querySelector(\"#grid\").selection.selected(function(index){document.querySelector(\"#grid\").getItem(index, function(err, item){return item.code;});});ajax.post('/grid', {id: 'grid', value: selection}, null);});});</script>", html
  end
